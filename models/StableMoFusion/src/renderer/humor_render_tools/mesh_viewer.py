...
class MeshViewer(object):
    ...
    def add_ground(
        ...
        for i in range(num_rows):
            for j in range(num_cols):
                ...
                cur_verts = np.array(
                    [
                        [start_loc[0], start_loc[1], z_offset],
                        [start_loc[0], start_loc[1] - tile_width,z_offset],
                        [start_loc[0] + tile_width, start_loc[1] - tile_width, z_offset],
                        [start_loc[0] + tile_width, start_loc[1], z_offset],
                    ]
                )
    ...
    def look_at(self, eye, target, up=[0, 1, 0]):
        """ Calcola la matrice di trasformazione look-at. """
        ...
        translation_matrix[:3, 3] = -np.array(eye)
    ...
    def animate(self, fps=30, start=None, end=None, progress_bar=tqdm):
        ...
        if self.animation_len == -1:
            ...
            if (self.follow_camera and not self.use_intrins):  # don't want to reset if we're going from camera view
                ...
                all_pos = np.array([np.mean(list(self.scene.meshes)[i].primitives[0].positions, axis=0) for i in range(len(self.scene.meshes))])
                mesh_mean = np.mean(all_pos, axis=0)

                # Posizione iniziale della camera
                camera_pose = self.get_init_cam_pose()
                camera_position = camera_pose[:3, 3]

                # Calcola il vettore di direzione dalla camera verso la mesh
                direction = mesh_mean - camera_position
                direction = direction / np.linalg.norm(direction)  # Normalizza il vettore
                direction = direction*(-1)


                # Crea un sistema di coordinate con 'direction' come asse z (forward)
                up = np.array([0, 0, 1])  # Definisci un vettore 'up' per la camera
                right = np.cross(up, direction)
                right = right / np.linalg.norm(right)
                up = np.cross(direction, right)

                # Aggiorna la rotazione della camera nel sistema di coordinate
                camera_pose[:3, 0] = right  # asse x
                camera_pose[:3, 1] = up     # asse y
                camera_pose[:3, 2] = direction  # asse z

                # Imposta la nuova posa della camera
                self.scene.set_pose(cam_node, camera_pose)
