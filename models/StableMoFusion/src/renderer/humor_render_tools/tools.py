...
def generate_color_sequence(n):
    # Definisco i 4 colori: blu, giallo, rosso, verde
    colors = np.array([
        [0.0390625, 0.4140625, 0.796875],  # Blu
        [0.03, 0.796875, 0.4], # verde
        [0.8, 0.8, 0.2],  # Giallo
        [0.796875, 0.4140625, 0.0390625],  # Rosso
    ])

    # Numero di vettori per colore
    M = n // 4

    # Genera la sequenza ripetendo ogni colore M volte
    color_sequence = np.repeat(colors, M, axis=0)

    diff = n - color_sequence.shape[0]
    if diff > 0:
        color_sequence = np.concatenate([color_sequence, np.repeat(np.array([colors[-1]]), diff, axis=0)], axis=0)
...
def viz_smpl_seq(
    ...
    if stick_humans:
        ...
        skel_color = skel_color=[0.7, 0.2, 0.2]
        joint_color=[8.0, 8.0, 8.0]
        joint_rad = 0.016
    ...
    if render_body or vtx_list is not None:
        ...
        body_mesh_seq = [
            trimesh.Trimesh(
                ...
                faces=faces,
                vertex_colors=np.tile(color_sequence[i], (nv, 1)),
                process=False,
            )
    ...
    if render_ground:
        ...
        mv.add_ground(
            ground_plane=ground_plane,
            xyz_orig=xyz_orig,
            color0=np.array([0.25, 0.25, 0.25]),
            color1=np.array([0.25, 0.25, 0.25]),
            alpha=ground_alpha,
            length=5,
            z_offset=-0.01
        )
