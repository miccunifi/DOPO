...
def render_animation(
    ...
    # Lista globale dei testi dei joint
    def update(frame):
        ...
        for index, (chain, color) in enumerate(zip(reversed(kinematic_tree), reversed(hcolors))):
            if not initialized:
                ...
                # Aggiungi numeri accanto ai joints
                for j in chain:
                    texts[j] = ax.text(
                        skeleton[j, x],
                        skeleton[j, y],
                        ...
                        fontsize=fontsize - 7,  # più piccoli
                        rotation=0,            # verticali
                        zorder=25
                    )
            else:
                # Aggiorna le linee
                ...
                # Aggiorna posizione dei numeri
                for j in chain:
                    texts[j].set_position((skeleton[j, x], skeleton[j, y]))
                    texts[j].set_3d_properties(skeleton[j, z])

        # Aggiorna spline della traiettoria
    ...
    if output == "notebook":
        ...
        try:
            ...
            mpl.rcParams['animation.ffmpeg_path'] = '/raid/users/micc-admin/gmacaluso/miniconda3/envs/stablediff/bin/ffmpeg'
            from matplotlib.animation import FFMpegWriter
            writer = FFMpegWriter(fps=20, codec='libx264')  # specify codec for mp4
