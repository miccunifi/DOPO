...
from .paramUtil import kit_raw_offsets, kit_kinematic_chain
...
def recover_root_rot_pos(data):
    ...
    r_rot_quat = torch.zeros(data.shape[:-1] + (4,), dtype=data.dtype).to(data.device) # todo mycode: dtype=data.dtype
    r_rot_quat[..., 0] = torch.cos(r_rot_ang)
    r_rot_quat[..., 2] = torch.sin(r_rot_ang)

    r_pos = torch.zeros(data.shape[:-1] + (3,), dtype=data.dtype).to(data.device) # todo mycode
