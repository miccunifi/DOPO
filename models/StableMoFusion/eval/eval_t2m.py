    ...
    for motion_loader_name, motion_loader in motion_loaders.items():
        ...
        with torch.no_grad():
            for idx, batch in enumerate(motion_loader):
                ...
                word_embeddings, pos_one_hots, _, sent_lens, motions, m_lens, __ = batch
                ...
                # dist_mat = np.nan_to_num(euclidean_distance_matrix(text_embeddings.cpu().numpy(), motion_embeddings.cpu().numpy()))
                # dist_mat = np.nan_to_num(euclidean_distance_matrix(motion_embeddings.cpu().numpy(), text_embeddings.cpu().numpy()))
...
def evaluate_fid(eval_wrapper,groundtruth_loader, activation_dict, file):
    ...
    with torch.no_grad():
        for idx, batch in enumerate(groundtruth_loader):
            ...
            if torch.isnan(motion_embeddings).any():# batch idx 19, motion n 38
                nan_mask = torch.isnan(motions)  # True where there are NaNs
                has_nan_per_element = nan_mask.view(64, -1).any(dim=1)  # Collapse last 2 dims and check if any NaN exists per [64] slice
                # Indices of elements with NaNs
                nan_indices = torch.nonzero(has_nan_per_element).squeeze()
                print("Indices with NaNs:", nan_indices)
                motion_embeddings = eval_wrapper.get_motion_embeddings(
                    motions=motions,
                    m_lens=m_lens
                )
...
def evaluation(eval_wrapper, motion_loaders ,eval_motion_loaders, log_file, replication_times, diversity_times, mm_num_times, run_mm=False):
