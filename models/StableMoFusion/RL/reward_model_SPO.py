...
class RewardModels():
    ...
    def init_models(self, dataset="humanml3d"):
        ...
        self.guo_forward = load_tm2t_model_easy(device="cpu", dataset=dataset) # humanml3d OR humanml3d_kitml_augmented_and_hn OR tmr_humanml3d_kitml_guoh3dfeats
...
def metric_fast(model, motions_guofeats,real_texts, c=None, device="cuda:0"):
    ...
    reward = tmr_plus_plus * reward_scale
...
def crop_motion_original_size_easy(sequences, sizes):
    motions = []
    for idx in range(sequences.shape[0]):
        x_start = sequences[idx]
        length = sizes[idx].item()
        x_start = x_start[:length]
...
def SPO_reward(sequences, sizes, real_texts, t, ):
    motions = crop_motion_original_size_easy(sequences, sizes)

    
    tmr, _ = metric_fast(reward_models.tmr_forward_complete, motions, real_texts)
