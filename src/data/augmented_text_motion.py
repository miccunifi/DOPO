...
from .collate import collate_text_motion_multiple_texts, collate_text_motion_multiple_texts_with_noise
...
class AugmentedTextMotionDataset(TextMotionDataset):
    def __init__(
        ...
        super().__init__(path, motion_loader, text_to_sent_emb=text_to_sent_emb, text_to_token_emb=text_to_token_emb, clip_embedder=clip_embedder,
                         split=split, min_seconds=min_seconds, max_seconds=max_seconds,
                         preload=False, tiny=tiny, with_noise=with_noise)

        if self.with_noise:
            self.collate_fn = partial(
                collate_text_motion_multiple_texts_with_noise,
    ...
    def load_keyid(self, keyid, text_idx=None, sent_emb_mode="first"):
        ...
        if sent_emb_mode == "first":
            sent_emb = self.text_to_sent_emb(text[0]) # TODO gestire il caso in cui text_to_sent_emb sia None 
