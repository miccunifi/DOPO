...
class AMASSMotionLoader:
    _normalization_warning_shown = False  # Flag di classe
    ...
    def __call__(self, path, start, end):
        ...
        if path not in self.motions:
            ...
            if not self.__class__._normalization_warning_shown:
                if self.normalizer is not None and not self.normalizer.disable:
                    print("⚠️  - Applying normalizer to motion loader")
                else:
                    print("⚠️  - No normalizer applied to motion loader")
                self.__class__._normalization_warning_shown = True
...
class Normalizer:
    ...
    def load(self):
        print(f"✅  - Loading normalizer from: {self.base_dir} with disable={self.disable}")
