...
class TMREvaluatorHandler(BaseEvaluator):
    """Handler per TMR Evaluator che carica il modello da checkpoint."""
    ...
    def encode(self, texts: List[str], motions: Union[torch.Tensor, List[np.ndarray]], lengths: torch.Tensor = None) -> torch.Tensor:
        ...
        for motion in motions:
            normed_motions.append(self.normalizer(motion))
        
        text_emb = self.encode_text(texts)
        motion_emb = self.encode_motion(normed_motions, lengths)
