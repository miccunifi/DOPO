...
from evaluators.DenseTMR.src.model import TextToEmb
...
def save_token_embeddings(
    ...
    print("Checking for texts that exceed 512 tokens...")
    long_texts = []
    for text in all_texts:
        tokens = tokenizer(text, return_tensors="pt")
        token_count = tokens['input_ids'].shape[1]
        if token_count > 512:
            long_texts.append((token_count, text))

    if long_texts:
        print(f"\nFound {len(long_texts)} texts exceeding 512 tokens:\n")
        for count, text in sorted(long_texts, reverse=True):
            print(f"[{count} tokens]: {text[:200]}...")
            print("-" * 80)
    else:
        print("No texts exceed 512 tokens.")
