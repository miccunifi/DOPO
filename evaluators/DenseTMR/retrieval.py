...
def compute_sim_matrix(model, dataset, keyids, batch_size=256):
    ...
    with torch.inference_mode():
        ...
        for data in tqdm(all_data_splitted, leave=False):
            ...
            ### Zero Noise
            # t = torch.zeros(motion_x_dict["x"].shape[0], device=motion_x_dict["x"].device)
            ###

            ### Noisy
            # t = torch.randint(0, 100, (motion_x_dict["x"].shape[0],), device=motion_x_dict["x"].device)
            t = torch.randint(95, 100, (1,), device=motion_x_dict["x"].device).repeat(motion_x_dict["x"].shape[0])

            motion_x_dict['t'] = t

            noise = torch.randn_like(motion_x_dict['x'])
            linear_scale = 1.0
            sqrt_alphas_cumprod = torch.tensor(np.load("sqrt_alphas_cumprod.npy")).to(motion_x_dict["x"].device)
            sqrt_one_minus_alphas_cumprod = torch.tensor(np.load("sqrt_one_minus_alphas_cumprod.npy")).to(motion_x_dict["x"].device)
